# -*- coding: utf-8 -*-
"""AI Summer SimCLR Resnet50 STL10.ipynb
...
# SimCLR in STL10 with Resnet50 AI Summer tutorial
...
class Hparams:
    def __init__(self):
        ...
        self.batch_size = 256
        self.lr = 7e-4 # for ADAm only
        self.weight_decay = 1e-6
        self.embedding_size= 128 # papers value is 128
        self.temperature = 0.5 # 0.1 or 0.5
        self.checkpoint_path = './SimCLR_ResNet50.ckpt' # replace checkpoint path here
...
def train_backbone():
    ...
    model = SimCLR_pl(train_config, model=resnet50(pretrained=False), feat_dim=1024)
    ...
    """## Save only backbone weights from Resnet50 that are only necessary for fine tuning"""

    model_pl = SimCLR_pl(train_config, model=resnet50(pretrained=False))
    model_pl = weights_update(model_pl, "SimCLR_ResNet50_sgd_.ckpt")

    resnet50_backbone_weights = model_pl.model.backbone
    print(resnet50_backbone_weights)
    torch.save({
                'model_state_dict': resnet50_backbone_weights.state_dict(),
                }, 'resnet50_backbone_weights.ckpt')
